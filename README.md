...
src/Support/Memory.php  пик RSS процесса — кросс-платформенно
...
* **каждый замер — новый процесс PHP**, иначе пик памяти достаётся в наследство от предыдущей
  библиотеки, а её классы уже загружены автолоадером;
* **память меряется по RSS процесса**, а не `memory_get_peak_usage()`. Счётчик PHP не видит
  аллокаций libxml: разбор листа на 12 МБ XML через `simplexml_load_string()` не меняет его вообще,
  а процесс при этом вырастает на ~313 МБ. По счётчику PHP библиотеки на DOM выглядят в разы
  легче, чем они есть. Источник цифры — `VmHWM` из `/proc/self/status` на Linux, иначе
  `getrusage()['ru_maxrss']`; если платформа пик RSS не отдаёт, отчёт откатывается на счётчик PHP
  и сообщает об этом;
...
* перед замером файл прогревается чтением с диска, вызывается `gc_collect_cycles()` и
  `memory_reset_peak_usage()`, снимается базовый уровень RSS;
